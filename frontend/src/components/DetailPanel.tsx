...
export default function DetailPanel({ snap }: Props) {
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      {/* Spike 경고 */}
      {snap.is_spike && (
        <div className="bg-red-900/40 border border-red-500/50 rounded-lg px-3 py-2 text-red-400 text-xs font-medium">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-slate-800 rounded-lg p-3">
          ...
          <p className={`text-base font-bold mt-1 ${snap.retransmit_count > 0 ? 'text-orange-400' : 'text-slate-100'}`}>
            {snap.retransmit_count}
          </p>
        </div>
        <div className="bg-slate-800 rounded-lg p-3">
          <p className="text-xs text-slate-400">샘플 수</p>
          <p className="text-base font-bold text-slate-100 mt-1">{snap.sample_count}</p>
        </div>
      </div>
    </div>
  )
