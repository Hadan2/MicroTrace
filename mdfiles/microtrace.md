...
현대의 마이크로서비스 아키텍처(MSA) 및 분산 시스템에서는 간헐적인 latency spike 하나가 사용자 오류, 타임아웃, 재시도 폭증으로 이어질 수 있습니다.

하지만 기존 상용 APM(Datadog, WhaTap 등) 솔루션은 비용과 운영 복잡도 때문에 소규모 팀에게 부담이 크고, 10초~1분 단위 집계 데이터만으로는 찰나의 spike와 그 시점의 시스템 상태를 함께 보기 어렵습니다.
...
| 항목 | 목표치 | 측정 방법 |
|---|---|---|
| Agent CPU 오버헤드 (평상시, 기본 감시) | < 1% | `perf stat` / `top` 비교 |
...
* **초기 MSA 백엔드 팀:** "service-a -> service-b 호출이 가끔 2초 걸린다" 같은 간헐적 지연의 원인을 빠르게 좁히고 싶은 팀
* **클라우드 인프라 (SRE):** 서비스 간 latency spike가 네트워크 문제인지 CPU/IO 문제인지 분류해야 하는 환경
* **외부 API 의존 서비스:** 결제/AI/서드파티 API 호출 시 timeout이 날 때, 내부 리소스 문제인지 외부 의존성 문제인지 먼저 가르고 싶은 팀

---

## 5. 시스템 아키텍처 (Architecture)

무거운 장기 저장보다 **상시 저비용 감시 + spike 시점 원인 분석**에 초점을 둔 3계층 아키텍처입니다.
...
[ 서비스 A ]  →  [ 서비스 B ]
     ...
     eBPF + 시스템 메트릭 상시 감시
      ...
      latency spike 감지?
         ...
         YES        NO → 요약 통계 유지
          │
   CPU / IO / Memory / TCP 지표 상관 분석
          │
   필요 시 kprobe/uprobe 추가 활성화
          │
      spike window 저장 및 전송
          │
       [ Go Collector ]
          │  WebSocket / API
       [ Dashboard ]
...
```
- 애플리케이션 코드 수정 없이 관측 가능
- 언어/프레임워크에 무관하게 공통된 커널 이벤트 수집 가능
- 네트워크 이벤트와 시스템 리소스 지표를 같은 시간축에서 결합 가능
- 특정 서비스/컨테이너에만 선택 적용하여 오버헤드 제어 가능
```

### 3단계 추적 구조

```
1단계: 기본 감시 (상시, 낮은 오버헤드)
   └── sock_ops로 서비스 간 latency, TCP 재전송 감시
   └── cgroup/procfs/PSI로 CPU throttling, IO wait, memory pressure 수집
   └── 평상시에는 이 저비용 지표들만 유지
...
2단계: kprobe (spike 감지 시 자동 활성화)
   ...
   └── "네트워크 문제인가, CPU/디스크 문제인가"를 더 정밀하게 판별
...
| | Datadog / New Relic | Cilium / Hubble | Pixie | MicroTrace |
|---|---|---|---|---|
| 핵심 포지션 | Full-stack observability | Kubernetes 네트워크/서비스 가시성 | K8s eBPF 자동 telemetry | 경량 latency root-cause profiler |
| 주력 대상 | 조직 전체 상시 모니터링 | Kubernetes 클러스터 | Kubernetes 클러스터 | Docker / VM 기반 소규모 팀 |
| 원인 분석 축 | APM + 로그 + 인프라 통합 | 네트워크/서비스 흐름 중심 | 프로토콜/리소스 자동 수집 | latency spike + CPU/IO/TCP 상관 분석 |
| 코드 수정 | 경우에 따라 필요 | 불필요 | 불필요 | 불필요 |
| 비용/복잡도 | 높음 | 중간 | 중간 | 낮음 |
| 강점 | 전사 관측 플랫폼 | K8s 네트워크 분석 | 자동 eBPF telemetry | 설치 가벼움, 좁은 문제에 빠른 진단 |

**핵심 차별점:**
- Datadog/New Relic 같은 대형 플랫폼보다 **설치와 운영 복잡도가 훨씬 낮음**
- Pixie/Cilium이 강한 Kubernetes 전제를 가지는 반면, MicroTrace는 **Docker/VM 환경을 1차 타겟**으로 둠
- 단순한 토폴로지 시각화보다 **"느린 호출의 원인이 네트워크인지 서버 리소스인지"를 먼저 갈라주는 데** 초점을 둠

---

## 8. 핵심 구현 포인트 (Technical Highlights)

* **sock_ops 기반 latency 감지:** kprobe 대비 낮은 오버헤드로 Keep-Alive 연결 위의 요청 단위 latency 측정.
* **시스템 리소스 상시 관측:** CPU throttling, IO wait, memory pressure를 latency 지표와 같은 시간축에서 확인.
* **동적 kprobe/uprobe 활성화:** 기본 감시로도 부족한 경우에만 더 깊은 커널/앱 레벨 추적을 활성화하여 오버헤드 최소화.
...
* **결과 검증:** 과부하 상태에서 특정 서비스 호출의 latency spike를 실시간 감지하고, CPU/IO/TCP 중 주요 원인 후보를 올바르게 좁히는지 확인.
...
| spike 감지 → 원인 후보 생성 | **3초 이내** CPU/IO/TCP 후보 제시 |
...
> **참고:** CPU/IO 지표 수집은 화면보다 먼저 의미가 있습니다. latency spike의 원인을 놓치지 않으려면 기본 리소스 지표는 상시 수집되어야 하며, UI는 그 상관관계를 더 이해하기 쉽게 보여주는 역할을 합니다.
