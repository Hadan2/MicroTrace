...
- StaticResolver: Docker 없는 환경(EC2)에서 IP→서비스명 수동 매핑
  - ✅ YAML 설정 파일 로딩 + `MICROTRACE_RESOLVER`/`MICROTRACE_HOSTS_FILE` 선택 구현
...
2. ~~**[버그] LatencyChart 전면 재작업**~~ ✅ uPlot 재작업 완료 (줌/팬 경계 붕괴 수정, range 전환 전체 리셋, Live 줌 유지+자동 추적 재개)
3. **Issue #6** StaticResolver + EC2 배포 + wrk 부하 테스트 + NFR 수치 실측 ← 다음
