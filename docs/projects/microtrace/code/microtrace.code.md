...
프로세스 3개가 stdin/stdout(JSON 한 줄)으로 연결된다. collector가 부모, 나머지 둘은 subprocess.

| 프로세스 | 언어 | 위치 | 역할 |
|---|---|---|---|
| `tcp_trace` | C/eBPF | `agent/` | sock_ops로 TCP RTT/재전송 수집 → JSON stdout |
...
[kernel] sock_ops 이벤트 → fill_event → ringbuf
  → agent/tcp_trace.c output_event: JSON 1줄 stdout
...
- ⚠️ `ForwardResource`가 `p.resources[svc]`를 갱신 → §5 cause 판별이 이걸 읽는다. **이름 키 일치 필수**: resource_agent의 컨테이너명(`containerName`)과 resolver가 만든 `ConnKey.Dst`가 같아야 dst 자원이 매칭된다(둘 다 Docker 컨테이너명이라 로컬에선 일치).
...
| Enrich | `enrich.go` `EnrichResolver` | Docker 미스 IP를 rDNS(`lookupAndParse`, eTLD+1)로 보완. 결과 캐시 |
| Static | `resolver.go` `StaticResolver` | 맵 기반(EC2/테스트). Docker 없을 때 fallback |

- 선택: `main.go`가 `NewDockerResolver` 성공→`NewEnrichResolver`로 감쌈, 실패→`NewStaticResolver(nil)`.
- ⚠️ `IsInternal`은 **Docker 캐시 기준**(rDNS 변환과 무관). external/internal 오판 방지용 — `stats.nodeType`이 이걸로 `dstType` 결정 → §5 cause의 `external` 분기에 직접 영향.
...
- `range`(1h/6h/24h/7d)는 `makeHistoryHandler`의 `rangeMap`으로 `from=now-dur` 하한만 정한다(`QueryHistory`의 `ts >= from`). **상한·개수 제한 없음** — 존재하는 데이터만큼만 반환하므로 데이터가 적으면 네 range가 같은 결과가 된다(정상). ⚠️ **향후 과제**: `LIMIT`/다운샘플링이 없어 7d에 데이터가 많이 쌓이면(최대 7일×1초≈60만 행) 조회·렌더가 느려질 수 있다. 분 단위 집계 등 다운샘플링 필요.
...
| 단일 writer 루프 | `Run` (★ gorilla/websocket은 동시 WriteMessage 불가 → 여기서만 write) |
| 업그레이드+등록+히스토리 전송 | `ServeWs` (`SetHistoryFn`으로 받은 `GetHistory` 호출) |
...
6. **이름 키 일치**(§6): resource_agent 컨테이너명 = resolver `ConnKey.Dst` 여야 cause가 dst 자원을 찾는다. EC2(StaticResolver)로 가면 매핑표를 양쪽 일관되게 둬야 함.
...
| 변경 | 건드리는 곳 | 안 건드리는 곳 |
|---|---|---|
| gRPC 전환(EC2 멀티호스트) | `agent.EventProvider`/`resource.ResourceProvider` 새 구현체 + `main.go` 선택 | stats, hub, model, store |
