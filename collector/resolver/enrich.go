// resolver/enrich.go
//
// EnrichResolver — 기본 resolver가 모르는 IP를 rDNS로 보완한다.
//
// 동작 순서:
//   1. base resolver로 조회 → 서비스명 반환 (내부 IP)
...
// EnrichResolver — 내부 IP는 서비스명으로, 외부 IP는 rDNS 도메인으로 변환
...
func NewEnrichResolver(base ServiceResolver) *EnrichResolver {
